...
export default function Home({ params }) {
  ...
  const handleClick = () => {
    console.log("clicked");
    if(params.userid == data?.loggedin)
    {
      router.push('/settings') //need to change this to edit
    }
    else{
      axios.post('/api/add_follower', {
        user_id: data?.loggedin,
        following_id: params.userid}).then((res) => {
          console.log(res)
              ...
              <div className="rounded-3xl bg-emerald-100 outline-double relative ml-4 flex justify-center flex-row w-full shadow-sky-100 hover:shadow-lg pb-2 ">
                ...
                <div className="flex justify-center flex-col max-w-7xl">
                  <button onClick={handleClick} class="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded">
                    {params.userid == data?.loggedin ? 'Edit Profile' : 'Follow'}
                  </button>
                </div>
              </div>
